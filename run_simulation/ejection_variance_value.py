...
from tools.functions import runkut4
from tools.printTool import initPrint

"""""

Run the simulations changing the ejection pressure value due to:
INIT_VALUE 
FINAL_VALUE
STEP_VALUE
...
def __run():
    ...
    ejection_variance = list(np.arange(INIT_VALUE, FINAL_VALUE, STEP_VALUE))
    ejection_variance_values = [(0,0,0)]


    for j in tqdm(range(len(ejection_variance)-1), desc="Processing" , ncols=100):
